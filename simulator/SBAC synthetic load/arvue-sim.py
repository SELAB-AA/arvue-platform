...
##                                print "All servers are closed, the new sessions are rejected (case 1)", self.sim.admissionController.arrived_sessions[0].name
                                ...
                                if random.uniform(0,1) < 0.4 or (self.sim.phase<3 or (self.sim.phase>3 and self.sim.phase<6) or (self.sim.phase>6 and self.sim.phase<8)):# Adnan: changed from self.sim.phase <3
                                ...
                                else: # if phase==3 or phase==6 or phase==8
...
class Arvue(Simulation,Logger):
     ...
     def run(self):
         ...
         self.report()
         # phase 6: all users disapear except 1
         self.phase=6
         print "Phase 6: scaling down to 1 sessions"
         while(self.nusers>1):
            self.simulate(until = self.now()+REPORT_SAMPLE_TIME)
            self._stop=False
            self.report()
         ...
         self.report()
         # phase 7: constant number of users 
         self.phase=7
         print "Phase 7: mantaining",1,"sessions for",CONSTANT_TIME*2,"seconds"
         t0=self.now()         
         while(self.now()<t0+CONSTANT_TIME*2):
            self.simulate(until = self.now()+REPORT_SAMPLE_TIME)
            self._stop=False
            self.report()
         self.report()

         # phase 8: all users disapear
         self.phase=8
         print "Phase 8: scaling down to 0 sessions"
         while(self.nusers>0):
            self.simulate(until = self.now()+REPORT_SAMPLE_TIME)
            self._stop=False
            self.report()
