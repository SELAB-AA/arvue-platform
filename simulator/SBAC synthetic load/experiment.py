...
# Arvue Controller
ARVUE_CONTROLLER_SAMPLE_INTERVAL = 1 #10 # 30 # read response time logs and CPU utilization after every t secs
AC_SAMPLE_INTERVAL = 30 # 1 # 30 # 5 # 10 # 1 #0.5 #  0.2 #0.5 # 0.1 # 1 #(IMPORTANT: 1 AC iteration admits only 1 or 0 session) check for any new or deferred sessions make AC decisions 

RESPONSE_TIME_THRESHOLD = 0.3
CPU_SCALEUP_THRESHOLD = 0.8 #0.9 #0.8 # 0.7 # 0.8 # when a server can be marked as SATURATED
MEM_SCALEUP_THRESHOLD = 0.8 #0.9 #0.8 # 0.7 # 0.8 # when a server can be marked as SATURATED

AC_CPU_SCALEUP_THRESHOLD = 0.8 #0.9 #0.8 # 0.7 #0.8 #0.9 # when a server can be marked as SATURATED for AC (admission control)
AC_MEM_SCALEUP_THRESHOLD = 0.8 #0.9 #0.8 #0.7 #0.8 #0.9 # when a server can be marked as SATURATED for AC (admission control)
...
CPU_SCALEDOWN_THRESHOLD = 0.2 #0.01 # 0.02 #0.2 # when a server can be marked as IDLE
MEM_SCALEDOWN_THRESHOLD = 0.2 #0.01 # 0.02 #0.2 # when a server can be marked as IDLE
...
SERVER_SHUTDOWN_DELAY=1 #20 COMMENT 1 sec is more realistic
SERVER_IDLE_INTERVALS_THRESHOLD=20 #10 # terminate a server only if it was idle for SERVER_IDLE_INTERVALS_THRESHOLD (intervals of ARVUE_CONTROLLER)
APP_IDLE_INTERVALS_THRESHOLD=50 #10 # unload an app only if it was idle for APP_IDLE_INTERVALS_THRESHOLD (intervals of ARVUE_CONTROLLER)
...
MeanCPUTime = 0.03 # 0.02 #0.01 #0.02 #0.030 #0.010 # 0.050  ## seconds

#Memory utilization per session
MeanMEMUtilPerSession = 0.008 #0.006 # MeanMEMUtil per session. with MeanMEMUtil=0.006 a server with total memory in 0..1 can handle approx. 166 simultaneous sessions.
...
MeanMEMUtilPerApp = 0.002 #0.003 # MeanMEMUtil per app. with MeanMEMUtil=0.003 a server with total memory in 0..1 can handle approx. 333 simultaneous apps.
...
# Simulation time
TARGET_N_SESSION=1000 #running with 1000 instead of 500
SESSION_UPRATE= 4 #2 #10 # 8 #4 #2#5 # session up rate in 1st peak
SESSION_UPRATE_PHASE2= 2 #1 #5 #4 #2 #1#3 # session up rate in 2nd peak
CONSTANT_TIME=30*60
